...
zScore3 <- data.frame(zLArmFmi,  zRArmFmi,     zLArmLmi, zRArmLmi,  zLLegFmi,  zRLegFmi,
                       zLLegLmi, zRLegLmi, zTrunkFmi, zTrunkLmi)
      

      
      colnames(zScore3) <- c( 'Z_FMI_LA', 'Z_FMI_RA', 
                              'Z_LMI_LA', 'Z_LMI_RA', 'Z_FMI_LL', 'Z_FMI_RL', 
                              'Z_LMI_LL',  'Z_LMI_RL', 'Z_FMI_TR', 'Z_LMI_TR') 
      
###
      ###
      ###CURRENTLY NOT CHANGING BETWQEEN GENDERS AND ETHNICITIES
      ###
...
#     
    output$text1 <- renderText({ 
      ...
      Gender <- input$Gender
      Race <- input$Race
      paste("Age/Gender/Race: ", Age, Gender, Race)
# 
# 
     })
